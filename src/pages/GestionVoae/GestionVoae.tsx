...
const GestionVoae: React.FC = () => {
  ...
  const handleFiltrar = () => {
    ...
    if (ambito_id) {
      const selectedAmbito = ambitos.find(
        (ambito) => ambito.id === parseInt(ambito_id)
      );
      if (selectedAmbito) {
        filtered = filtered.filter(
          (actividad) => actividad.ambito === selectedAmbito.nombre_ambito
        );
      }
    }
    ...
    if (carrera_id) {
      const selectedCarrera = carreras.find(
        (carrera) => carrera.id === parseInt(carrera_id)
      );
      if (selectedCarrera) {
        filtered = filtered.filter(
          (actividad) => actividad.carrera === selectedCarrera.nombre_carrera
        );
      }
    }
